...
module.exports=function(){
 return `<div id="seeya-makers-letter">
  <section class="letter-page" aria-labelledby="letter-title">
    <div class="letter-page-heading"><span>A LETTER FOR YOU · 제작자의 편지</span><h1 id="letter-title">${esc(letter.heading)}</h1></div>
    <article class="letter-paper" aria-labelledby="letter-greeting">
      ...
      <h2 id="letter-greeting">${esc(letter.greeting)}</h2>
      <div class="letter-writing">${letter.paragraphs.map(lines=>`<p>${lines.map(esc).join('<br> ')}</p>`).join('\n')}</div>
      <footer class="letter-signature"><span>${esc(letter.closing)}</span><div>${esc(letter.signature)}</div>${heart}</footer>
