...
function printQuestions(order,name='',bank=data){return require('./exam-layout').papers((q,i)=>`<section class="exam-print-question"><h3>${i+1}. ${esc(q.prompt)} <small>[4점]</small></h3>${q.type==='choice'?(order?order[i]:q.choices.map((_,n)=>n)).map((v,n)=>`<p>${['①','②','③','④'][n]} ${esc(q.choices[v])}</p>`).join(''):'<p class="exam-print-answer">답: <span></span></p>'}</section>`,name,bank);}
module.exports={data,banks,esc,normalize,correct,answered,grade,shuffledChoices,printQuestions};
