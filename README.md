...
v4.94 이후 변경 사항을 모두 포함한 누적 업데이트입니다. 모의고사 이미지·SEEYA PLAY 카드·오늘 날짜의 씨야·TODAY 카드 폭·로컬 배치파일을 함께 적용합니다. 중간 버전인 v4.97을 건너뛰고 v4.98만 적용했어도 이번 ZIP을 한 번 덮어쓰면 됩니다.

## 이번 업데이트 적용

1. GitHub Desktop에서 **Fetch origin → Pull origin**으로 최신 파일을 받습니다.
2. `SEEYA_v4_99_UPLOAD.zip`의 내용물을 저장소 최상위 폴더에 같은 경로로 복사합니다.
3. 변경 내역을 확인하고 **Commit → Push**합니다. 기존 Actions의 사이트 빌드와 Pages 배포 완료를 기다립니다.

이번 UPLOAD는 **v4.94~v4.98에서 바로 적용할 수 있는 누적본**입니다. 새 모의고사 이미지 파일과 관련 페이지·스타일·날짜 기능·배치파일이 모두 들어 있습니다. 기존 아카이브·뉴스·사진·방문자 데이터와 생성 HTML은 덮어쓰지 않습니다. 이미지 주소에는 버전 표시를 붙여 이전 응답이 캐시에 남아도 새 주소로 요청합니다. FULL_BACKUP은 전체 보관용이며 로컬 확인을 위해 풀 필요는 없습니다.
