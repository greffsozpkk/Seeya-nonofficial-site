...
멤버 소개의 사인을 모바일·PC에서 다르게 배치했습니다. 모바일과 넓은 PC 모두 프로필 사진·이름 오른쪽 여백에 사인을 배치합니다. 851~1200px의 좁은 PC 카드에서는 이름 아래로 내려 크기를 확보하고 겹침을 피합니다. v4.92의 아카이브 919건을 모두 유지합니다.

## 이번 업데이트 적용

1. GitHub Desktop에서 **Fetch origin → Pull origin**으로 최신 파일을 받습니다.
2. `SEEYA_v4_94_UPLOAD.zip`의 내용물을 저장소 최상위 폴더에 같은 경로로 복사합니다.
3. 변경 내역을 확인하고 **Commit → Push**합니다. 기존 Actions의 사이트 빌드와 Pages 배포 완료를 기다립니다.

UPLOAD는 문서·버전·멤버 페이지·스타일 소스 6개 파일입니다. 아카이브·뉴스·사진·방문자 데이터와 생성 HTML은 덮어쓰지 않습니다. v4.92 적용본 기준 업데이트이며 FULL_BACKUP에는 전체 소스·이미지·빌드 산출물이 있습니다. 이전 통합 정리를 완료했다면 정리 배치파일을 다시 실행할 필요는 없습니다.
