...
TODAY의 “오늘 날짜의 씨야” 카드에 중복 적용된 좌우 바깥 여백을 제거했습니다. 아래 운세·추천곡 영역과 양쪽 끝선이 일치하며, 모바일에서는 기존 카드와 같은 폭으로 표시됩니다. 위치는 TODAY에 유지합니다.

## 이번 업데이트 적용

1. GitHub Desktop에서 **Fetch origin → Pull origin**으로 최신 파일을 받습니다.
2. `SEEYA_v4_98_UPLOAD.zip`의 내용물을 저장소 최상위 폴더에 같은 경로로 복사합니다.
3. 변경 내역을 확인하고 **Commit → Push**합니다. 기존 Actions의 사이트 빌드와 Pages 배포 완료를 기다립니다.

이번 UPLOAD는 v4.97 적용 폴더를 위한 카드 폭 수정본입니다. 스타일·버전·문서 5개 파일만 포함합니다. 기존 아카이브·뉴스·사진·방문자 데이터와 생성 HTML은 덮어쓰지 않습니다. v4.97의 모의고사 이미지·과거 오늘 기록·새 배치파일은 그대로 사용합니다. FULL_BACKUP은 전체 보관용이며, 로컬 확인을 위해 다시 풀 필요는 없습니다.
...
`scripts/check.js`는 빌드 재현성·13개 경로 본문·JS 문법·로컬 링크와 아카이브 검사를 실행합니다. `scripts/check-archive.js`는 ID·날짜·출처, 필터·페이지 이동 및 주요 회차를 검사합니다. `scripts/check-on-this-day.js`는 한국 자정·윤년·날짜 일치·예정 및 날짜 미확인 기록 제외·게시일 표시를 검사하며 전체 검사에도 포함됩니다. 별도 브라우저 검사는 Edge와 Playwright 설치 환경에서 진행합니다.
