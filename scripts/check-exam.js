...
const {data,banks,correct,answered,grade,shuffledChoices,printQuestions}=require('../src/shared/exam');
...
assert(correct(data.questions[20],'  내 잘못 입니다  '));assert(correct(data.questions[24],'i BeLiEvE'));assert(!correct(data.questions[24],'우리들'));assert(!correct(data.questions[20],'잘못입니다'));assert(!correct(data.questions[24],' '));
...
for(const bank of banks){
 ...
 const answers={};for(const q of bank.questions){assert(q.prompt&&q.explanation&&q.source.startsWith('https://'));if(q.type==='choice'){assert.equal(q.choices.length,4);assert.equal(new Set(q.choices).size,4);assert(Number.isInteger(q.answer)&&q.answer>=0&&q.answer<4);}answers[q.id]=q.answer;assert(correct(q,q.answer));for(const alias of q.aliases||[])assert(correct(q,alias));}
