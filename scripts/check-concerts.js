...
const daegu=fan.events.find(e=>e.label==='대구');assert.equal(daegu.videos.length,5);assert.equal(daegu.songs.length,7);assert(!daegu.references.some(s=>s.url.includes('docs.google.com')));assert.deepEqual(daegu.songs,archive.find(r=>r.id===daegu.id).songs);
