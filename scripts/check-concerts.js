...
const daegu=fan.events.find(e=>e.label==='대구');assert.equal(daegu.videos.length,6);assert(daegu.videos.some(s=>s.url.endsWith('dW8vBqXwWGE')));assert.equal(daegu.date,'2026-09-12');assert.equal(daegu.songs.length,7);assert(!daegu.references.some(s=>s.url.includes('docs.google.com')));assert.deepEqual(daegu.songs,archive.find(r=>r.id===daegu.id).songs);
